import { cssFromSource } from "https://esm.sh/@knighted/css/browser";
  ...
  try {
    ...
    try {
      ...
      if (secondError instanceof Error) {
        throw new Error(
          `Unable to load Lightning CSS WASM: ${secondError.message}`,
          { cause: secondError },
        );
      }
      if (firstError instanceof Error) {
        throw new Error(
          `Unable to load Lightning CSS WASM: ${firstError.message}`,
          { cause: secondError },
        );
      }
...
const evaluateUserModule = async (helpers = {}) => {
  const userCode = jsxEditor.value
    .replace(
      /^\s*export\s+default\s+function\b/gm,
      "__defaultExport = function",
    )
